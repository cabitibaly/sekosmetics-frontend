...
const KitNavbar = () => {
    return (
        <nav className={`fixed z-40 left-1/2 -translate-1/2 top-12 border border-red-4 bg-red-1 py-3 px-8 w-[90%] rounded-3xl flex items-center justify-between gap-4 max-[896px]:hidden`}>
            ...
            <Link href={"/creation-kit"} className="bg-red-8 rounded-full font-bold text-gris-12 tetx-base py-2 px-4 cursor-pointer ease-in-out transition duration-300 border border-transparent hover:text-red-8 hover:bg-red-1 hover:border-red-6
                max-lg:text-sm">
                Confirmer
            </Link>
        </nav>
