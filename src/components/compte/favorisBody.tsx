...
const FavorisBody = () => {
    ...
    const { articles, isFetchingNextPage, fetchNextPage, hasNextPage } = useGetLesArticlesPagine(8, "", "", debouceValue.trim() || "");
    const { favorisArticles, refetch } = useGetLesFavoris();

    return (
        <div className={`overflow-x-hidden px-[100px] pt-32 py-6 w-screen h-screen flex items-start justify-center gap-4 max-xl:px-[30px] max-896:flex-wrap max-896:!pb-36 max-896:!px-4 max-896:pt-20 max-xs:!pb-24`}>
            <div className='w-4/5 h-full flex items-start justify-center gap-8 max-lg:w-[90%] max-896:!w-full'>
                ...
                <div className='border border-red-3 p-4 rounded-3xl bg-red-1 w-3/5 h-full flex flex-col items-center justify-start gap-4 max-896:w-full max-896:bg-transparent max-896:border-none max-896:p-0'>
                    ...
                    <div className="overflow-auto flex flex-col items-center justify-start gap-4 w-full">
                        <div className="w-full grid grid-cols-3 items-center justify-start gap-4 max-[1380px]:grid-cols-2 max-896:!grid-cols-4 max-sm:!grid-cols-3 max-xs:!grid-cols-2">
                            {
