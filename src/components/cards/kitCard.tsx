...
const KitCard = ({id}: Props) => {
    const { supprimerLigneKit } = useKit()
    ...
    return (
        <div className="relative border border-red-6 p-4 rounded-3xl w-[87%] carousel-item flex-col items-center justify-start gap-3 max-896:flex-row max-896:w-2/3 max-896:h-auto max-xs:w-[88%]">
            ...
            <div className="w-full flex flex-col items-start justify-center gap-4">
                ...
                <span className="text-red-8 text-2xl text-left font-bold max-xl:text-xl max-lg:text-base max-896:!text-xl max-sm:!text-lg max-xs:!text-base max-[360px]:!text-xs">{article?.prix.toLocaleString("fr-FR")} FCFA</span>
            </div>
